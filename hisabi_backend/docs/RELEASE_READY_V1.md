...
Target production host: `https://expense.yemenfrappe.com`

## Pre-Release
1. Verify site encryption key is configured:
```bash
cd /home/frappe/frappe-bench
bench --site expense.yemenfrappe.com console <<'PY'
import frappe
...
bench --site expense.yemenfrappe.com clear-cache
```
...
```bash
curl -fsS https://expense.yemenfrappe.com/api/method/hisabi_backend.api.v1.health.ping
curl -fsS https://expense.yemenfrappe.com/api/method/hisabi_backend.api.v1.health.diag
```
...
```bash
cd /home/frappe/frappe-bench
bench --site expense.yemenfrappe.com logs --web --worker --short
```
...
- Run `bench --site expense.yemenfrappe.com migrate` if rollback includes schema change.
